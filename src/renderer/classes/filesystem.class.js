...
const pathResolve = (base, rel) => {
    if (rel.startsWith('/')) return rel;
    const combined = pathJoin(base, rel);
...
class FilesystemDisplay {
    constructor(opts) {
        ...
        this.followTab = () => {
            ...
            // Listen for CWD changes via IPC
            this._cwdUnsub = window.electronAPI.onCwdChanged((id, cwd) => {
                if (this._noTracking) return false;
        ...
        this.readDevices = async () => {
            ...
            let blocks;
            try {
                blocks = await window.electronAPI.getBlockDevices();
            } catch (e) {
                console.warn("Failed to read block devices:", e);
                return;
            }
            let devices = [];
        ...
        this.render = async (originBlockList, isDiskView) => {
            ...
            blockList.forEach((e, blockIndex) => {
                    ...
                    switch(e.type) {
                        ...
                        default:
                            if (this.fileIconsMatcher) {
                                ...
                                icon = { width: 24, height: 24, svg: '<path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>' };
                                type = e.type;
                            }
                            break;
                    }
                ...
                filesDOM += `<div class="fs_disp_${e.type}${hidden} animationWait" data-block-index="${blockIndex}">
            ...
            this._attachFileClickHandlers(blockList);

            const spaceBar = document.getElementById("fs_space_bar");
            if (spaceBar) {
                if (this.filesContainer.getAttribute("class").endsWith("disks")) {
                    spaceBar.onclick = () => window.fsDisp.render(window.fsDisp.cwd);
                } else {
                    spaceBar.onclick = null;
                }
            }
        ...
        this.reCalculateDiskUsage = async (dirPath) => {
            this.fsBlock = null;
            ...
            if (d) {
                d.forEach(fsBlock => {
                    if (dirPath.startsWith(fsBlock.mount)) {
        ...
        this.openFile = async (name, path, type) => {
            ...
            switch (filetype) {
                ...
                default:
                    ...
                    let data;
                    try {
                        data = await window.electronAPI.readFile(block.path, 'utf-8');
                    } catch (err) {
                        new Modal({
                            type: "info",
    ...
    _attachFileClickHandlers(blockList) {
        ...
        this._clickHandler = (event) => {
            const fileEntry = event.target.closest('[data-block-index]');
            if (!fileEntry) return;

            const blockIndex = parseInt(fileEntry.dataset.blockIndex, 10);
            const e = blockList[blockIndex];
            if (!e) return;

            const cwdEntry = this.cwd[blockIndex];
            if (!cwdEntry) return;

            const isWin = navigator.platform.includes('Win');

            // Ctrl+click: open in OS default app
            if (event.ctrlKey) {
                window.electronAPI.openPath(cwdEntry.path);
                return;
            }

            // Shift+click: insert path in terminal (no execute)
            if (event.shiftKey) {
                window.term[window.currentTerm].write('"' + cwdEntry.path + '"');
                return;
            }

            // showDisks: read devices
            if (e.type === 'showDisks') {
                this.readDevices();
                return;
            }
            ...
            if (e.type === 'disk' || e.type === 'rom' || e.type === 'usb') {
                if (this._noTracking) {
                    ...
                    if (isWin) {
                        ...
                        window.term[window.currentTerm].writelr('cd "' + cwdEntry.path + '"');
                    }
                }
                return;
            }
            ...
            if (e.type === 'video' || e.type === 'audio' || e.type === 'image') {
                ...
                this.openMedia(blockIndex);
                window.audioManager.folder.play();
                return;
            }

            // file: open file viewer
            if (e.type === 'file') {
                this.openFile(blockIndex);
                return;
            }
        };

        this._clickHandlerTarget = container;
