    ...
    constructor(parentId) {
        ...
        window.electronAPI.getCpuInfo().then(data => {
            ...
            innercontainer.innerHTML = `<h1>CPU USAGE<i>${cpuName}</i></h1>
                ...
                <div>
                    <div>
                        <h1>${(navigator.platform.includes('Win')) ? "CORES" : "TEMP"}<br>
                        <i id="mod_cpuinfo_temp">${(navigator.platform.includes('Win')) ? data.cores : "--°C"}</i></h1>
                    </div>
                    <div>
                        <h1>SPD<br>
            ...
            this.updateCPUload();
            if (!navigator.platform.includes('Win')) {this.updateCPUtemp();}
